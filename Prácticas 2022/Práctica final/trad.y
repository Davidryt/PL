...
%type   <cadena>    Decl_Funciones_Main     Decl_Variables      r_funciones
                    ...
                    op_logica   op_comparacion      args_func args_r
...
Decl_Funciones: IDENTIF '(' ')' '{' cuerpo_main '}'     
            ...
            |   IDENTIF '(' l_arg ')' '{' cuerpo_main '}'     
                                                    { sprintf (temp, "(defun %s (%s) \n%s\n)\n", $1,$3, $6) ; 
                                                        $$ = genera_cadena (temp);}
            |   IDENTIF '(' l_arg ')' '{' cuerpo_main retu '}'
                                                    { sprintf (temp, "(defun %s (%s)\n%s\n(return-from %s %s))\n",$1,$3,$6,$1,$7) ; 
                                                        $$ = genera_cadena (temp);}
...
sentencia:      asignacion                          { sprintf (temp, "%s", $1) ;
            ...
            |   for                                 { sprintf (temp, "%s", $1);
                                                        $$ = genera_cadena (temp) ; }
            |   IDENTIF '(' args_func ')' ';'       { sprintf (temp, "(%s %s)", $1, $3);
                                                        $$ = genera_cadena (temp) ; }
;
...
args_func:      expresion args_r
                                                    { sprintf (temp, "%s %s", $1, $2);
            									        $$ = genera_cadena (temp);}
            |   args_r              { sprintf (temp, "%s", $1);
            									        $$ = genera_cadena (temp);}
;

args_r:         ',' expresion r_argu
