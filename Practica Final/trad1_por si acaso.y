...
axioma:         globales main                                   { printf ("%s%s\n(main)\n", $1.code, $2.code) ; }
            |   main                                            { printf ("%s\n(main)\n", $1.code) ; }
            ;

globales:       declaracion ';' r_globales                      { sprintf (temp, "%s \n%s", $1.code, $3.code) ;
                                                                    $$.code = gen_code (temp) ; } 
            ;

r_globales:     
                /* Lambda */                                    { $$.code = ""; }
            |   declaracion ';' r_globales                      { sprintf (temp, "%s \n%s", $1.code, $3.code) ;
                                                                    $$.code = gen_code (temp) ; } 
            ;

main:           MAIN '(' ')' '{' body '}'                       { sprintf (temp, "(defun main ()\n%s) ", $5.code) ;  
                                                                    $$.code = gen_code (temp) ; }
            ;
...
sentencia:     declaracion                                      { $$ = $1 ; }
            | '$' '(' expresion r_sentencia ')'                 { sprintf (temp, "(print %s) %s", $3.code, $4.code) ;  
                                                                    $$.code = gen_code (temp) ; }
            ;

r_sentencia:  ',' expresion r_sentencia                         { sprintf (temp, "(print %s) %s", $2.code, $3.code) ;  
                                                                    $$.code = gen_code (temp) ;}
            |   /* Lambda */                                    { $$.code = ""; }
            ;

declaracion:    INTEGER IDENTIF '=' NUMBER r_declaracion        { sprintf (temp, "(setq %s %d) %s", $2.code, $4.value, $5.code) ; 
                                                                    $$.code = gen_code (temp) ; }
...
r_declaracion:  ',' r_declaracion                               { sprintf (temp, " %s", $2.code) ;  
                                                                    $$.code = gen_code (temp) ;}
            |   IDENTIF '=' NUMBER r_declaracion                { sprintf (temp, "(setq %s %d) %s", $1.code, $3.value, $4.code) ; 
                                                                    $$.code = gen_code (temp) ; }
